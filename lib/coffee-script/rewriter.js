...
(function() {
  ...
  exports.Rewriter = (function() {
    ...
    Rewriter.prototype.addImplicitFieldValues = function() {
      var action, arrays, calls, level, params, scanner, stack;
      stack = [];
      calls = [];
      params = [];
      arrays = [];
      action = function(token, i) {
        ...
        if (token[0] === '@') {
          ...
          this.tokens.splice(i + 1, 0, this.generate(':', ':', token[2]));
          this.tokens.splice(i + 2, 0, this.generate(token[0], token[1], token[2]));
          this.tokens.splice(i + 3, 0, this.generate(id[0], id[1], id[2]));
          return;
        }
        this.tokens.splice(i + 1, 0, this.generate(':', ':', token[2]));
        return this.tokens.splice(i + 2, 0, this.generate(token[0], token[1], token[2]));
      };
      level = 0;
      scanner = function(token, i, tokens) {
        var arrayIndex, endBrace, endBraces, inArray, inParams, paramIndex, r1, r2, r3, stackIndex, tag;
        tag = token[0];
        if (tag === '{') {
          stack.push({
            i: i,
        ...
        if (!(tag === '{' || ((!inParams) && (!inArray) && stack.length && tag === ','))) {
          return 1;
        }
        ...
        endBrace = i;
        endBraces = 0;
        while (endBrace < this.tokens.length - 1) {
          tag = this.tag(++endBrace);
          if (tag === '{') {
            ++endBraces;
          }
          if (tag === '}') {
            ...
            this.tokens.splice(endBrace, 1);
            this.tokens.splice(i, 0, this.generate('}', '}', token[2]));
            return 1;
          }
        }
        return 1;
      };
      return this.scanTokens(scanner);
    };
